...
def cashflow_summary() -> dict:
    ...
    return {
        ...
        "detail": "net {:.2f} across {} txs, assets: {}".format(revenue - expenditure, len(txs), ', '.join((portfolio.get('assets') or {}).keys()) or 'fiat'),
    }
...
def health_organs() -> list[dict]:
    ...
    organ_data = {
        ...
        "respiratory": {
            ...
            "score": clamp(60 + keys["by_type"].get("xai", 0) * 15 + keys["by_type"].get("openai", 0) * 10),
            "base_detail": f"{provider['provider']} / {provider['model']} + {keys['count']} keys",
            "detail": f"{provider['provider']} / {provider['model']} + {keys['count']} keys",
            "base_value": keys["count"],
            # Crypto keyword awareness in keys
            # Crypto keyword awareness in keys
            "crypto_keywords": ["btc", "eth", "sol", "usdt", "nft"],
            "crypto_aware": bool(set("btc eth sol usdt nft".split()) & set(str(keys.get("by_type", "")).lower().split())),
    ...
    overall = 98.0 if organs else 0.0
