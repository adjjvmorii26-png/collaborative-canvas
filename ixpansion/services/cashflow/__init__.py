...
class Portfolio:
    ...
    nfts: Dict[str, Dict[str, Any]]  # nft_id -> {name, rarity, value}
    ...
    def total_value(self, price_lookup: Dict[str, Decimal]) -> Decimal:
        ...
        for nft_id, nft in self.nfts.items():
            nft_value = nft.get("value", "0")
            if isinstance(nft_value, str):
                total += Decimal(nft_value)
            elif isinstance(nft_value, Decimal):
                total += nft_value
        return total
...
class CashFlowTracker:
    ...
    def __init__(self, base_dir: str = "ixpansion/content_output/cashflow") -> None:
        ...
        self._price_lookup: Dict[str, Decimal] = {}
        self._load()

    def _load(self) -> None:
        """Load existing data from disk."""
        # Load transactions - handle missing/corrupt files gracefully
        self._transactions = []
        ...
        # Load portfolio - handle missing/corrupt files gracefully
        ...
        # Load price lookup - handle missing/corrupt files gracefully
        self._price_lookup = {}
            ...
            try:
                ...
                if isinstance(data, dict):
                    for k, v in data.items():
                        try:
                            self._price_lookup[k] = Decimal(v)
    ...
    def update_portfolio(
        ...
        if nft_data:
            ...
            self._portfolio.nfts[nft_id] = {
                ...
                "value": str(nft_data.get("value", Decimal("0"))),
